...
public:
	...
	String<STRA> parse (CREF<String<STRA>> def) const override {
		if (!exist ())
			return def ;
		try {
			return StringProc::stra_from_stru (self.mThis->mTree[self.mIndex].mValue) ;
		} catch (CREF<Exception> e) {
			noop (e) ;
		}
		return def ;
	...
	String<STRW> parse (CREF<String<STRW>> def) const override {
		if (!exist ())
			return def ;
		try {
			return StringProc::strw_from_stru (self.mThis->mTree[self.mIndex].mValue) ;
		} catch (CREF<Exception> e) {
			noop (e) ;
		}
		return def ;
	...
	String<STRU16> parse (CREF<String<STRU16>> def) const override {
		if (!exist ())
			return def ;
		try {
			return StringProc::stru16_from_stru8 (self.mThis->mTree[self.mIndex].mValue) ;
		} catch (CREF<Exception> e) {
			noop (e) ;
		}
		return def ;
	...
	String<STRU32> parse (CREF<String<STRU32>> def) const override {
		if (!exist ())
			return def ;
		try {
			return StringProc::stru32_from_stru8 (self.mThis->mTree[self.mIndex].mValue) ;
		} catch (CREF<Exception> e) {
			noop (e) ;
		}
		return def ;
