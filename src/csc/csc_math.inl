...
public:
	...
	BOOL all_of (CR<BOOL> a ,CR<WrapperLayout> b) const override {
		if (!a)
			return FALSE ;
	...
	BOOL any_of (CR<BOOL> a ,CR<WrapperLayout> b) const override {
		if (a)
			return TRUE ;
		auto &&rax = keep[TYPE<Wrapper<BOOL>>::expr] (b) ;
		for (auto &&i : range (0 ,rax.rank ())) {
	...
	VAL32 max_of (CR<VAL32> a ,CR<WrapperLayout> b) const override {
		return max_of_impl (a ,b) ;
	}

	VAL64 max_of (CR<VAL64> a ,CR<WrapperLayout> b) const override {
		return max_of_impl (a ,b) ;
	}

	FLT32 max_of (CR<FLT32> a ,CR<WrapperLayout> b) const override {
		return max_of_impl (a ,b) ;
	}

	FLT64 max_of (CR<FLT64> a ,CR<WrapperLayout> b) const override {
		return max_of_impl (a ,b) ;
	}

	template <class ARG1>
	forceinline ARG1 max_of_impl (CR<ARG1> a ,CR<WrapperLayout> b) const {
		ARG1 ret = a ;
	...
	VAL32 min_of (CR<VAL32> a ,CR<WrapperLayout> b) const override {
		return min_of_impl (a ,b) ;
	}

	VAL64 min_of (CR<VAL64> a ,CR<WrapperLayout> b) const override {
		return min_of_impl (a ,b) ;
	}

	FLT32 min_of (CR<FLT32> a ,CR<WrapperLayout> b) const override {
		return min_of_impl (a ,b) ;
	}

	FLT64 min_of (CR<FLT64> a ,CR<WrapperLayout> b) const override {
		return min_of_impl (a ,b) ;
	}

	template <class ARG1>
	forceinline ARG1 min_of_impl (CR<ARG1> a ,CR<WrapperLayout> b) const {
		ARG1 ret = a ;
		auto &&rax = keep[TYPE<Wrapper<ARG1>>::expr] (b) ;
		for (auto &&i : range (0 ,rax.rank ())) {
			if (ret <= rax[i])
...
public:
	...
	void initialize (CR<JetLayout> that) override {
	...
	void once (VR<Ref<JetNode>> node ,CR<WrapperLayout> params) const {
		if (!node.exist ())
			return ;
