...
public:
	...
	FLAG load (CREF<Clazz> clazz) const override {
		Scope<Mutex> anonymous (fake->mRoot->mMutex) ;
		auto rax = Set<Clazz> () ;
		fake->mRoot->mClazzSet.get (rax) ;
		FLAG ret = rax.map (clazz) ;
		fake->mRoot->mClazzSet.set (rax) ;
